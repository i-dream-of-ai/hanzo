    ...
    def register_tools(self, mcp_server: FastMCP) -> None:
        ...
        async def read_multiple_files(paths: list[str], ctx: MCPContext) -> str:
            ...
            for i, path in enumerate(paths):
                ...
                if not self.permission_manager.is_path_allowed(path):
                    results.append(f"{path}: Error - Access denied - path outside allowed directories")
                    continue
        ...
        # Edit file tool
        @mcp_server.tool()
        async def edit_file(path: str, edits: list[dict[str, str]], dry_run: bool , ctx: MCPContext) -> str:
                        ...
                        if old_text in modified_content:
                            ...
                            for i in range(len(content_lines) - len(old_lines) + 1):
                                ...
                                matches = all(
                                    ...
                                    for old_line, content_line in zip(old_lines, current_chunk)
                                )
        ...
        # Search files tool
        @mcp_server.tool()
        async def search_files(path: str, pattern: str, exclude_patterns: list[str], ctx: MCPContext) -> str:
            ...
            try:
                ...
                # Build list of excluded patterns
                exclusion_patterns = [
                    pattern.lower() for pattern in exclude_patterns
                ]
                
                # Function to check if a path should be excluded
                def is_excluded(entry_path: Path) -> bool:
                    path_str = str(entry_path).lower()
                    
                    for excl_pattern in exclusion_patterns:
                        if excl_pattern in path_str:
                            return True
                    
                    return False
                
                # Recursive search function
                async def search_recursive(current_path: Path) -> None:
                    # Skip if not allowed
                    if not self.permission_manager.is_path_allowed(str(current_path)):
                        return
                    ...
                    try:
                        for entry in current_path.iterdir():
                            # Skip if excluded or not allowed
                            if is_excluded(entry) or not self.permission_manager.is_path_allowed(str(entry)):
                                continue
                    ...
                    try:
                        ...
                        if count > 0:
                            ...
                            if not dry_run:
                                ...
                                # Update document context
                                self.document_context.update_document(str(file_path), new_content)
                    except UnicodeDecodeError:
                        # Skip binary files
                        continue
                    except Exception as e:
