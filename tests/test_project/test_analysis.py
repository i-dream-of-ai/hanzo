...
class TestProjectAnalyzer:
    ...
    async def test_analyze_python_dependencies(self, analyzer, test_project_dir):
        ...
        with patch.object(analyzer.command_executor, "execute_script_from_file",
    ...
    @pytest.mark.asyncio
    async def test_analyze_python_dependencies_failure(self, analyzer, test_project_dir):
...
class TestProjectManager:
    ...
    def test_initialization(self, document_context, permission_manager, project_analyzer):
    ...
    def test_set_project_root_disallowed(self, project_manager, temp_dir):
        ...
        assert project_manager.project_root is None
    
    def test_detect_programming_languages(self, project_manager, test_project_dir, permission_manager):
    ...
    def test_detect_project_type(self, project_manager, test_project_dir, permission_manager):
        ...
        with patch.object(project_manager, "_read_json", return_value={}), \
             patch.object(project_manager, "_read_lines", return_value=["pytest==7.3.1", "mcp>=1.3.0"]):
            # Detect project type
    ...
    @pytest.mark.asyncio
    async def test_analyze_project(self, project_manager, test_project_dir, permission_manager):
        ...
        # Mock methods
        project_manager.detect_programming_languages = MagicMock(return_value={"Python": 3, "Markdown": 1})
        project_manager.detect_project_type = MagicMock(return_value={"type": "web-backend", "frameworks": ["Django"]})
        
        # Mock project_analyzer methods
        project_manager.project_analyzer.analyze_project_structure = AsyncMock(return_value={
            "file_count": 10,
            "directory_count": 5,
            "extensions": {".py": {"count": 3}}
        })
        project_manager.project_analyzer.analyze_python_dependencies = AsyncMock(return_value={
            "requirements_files": ["requirements.txt"],
            "imports": ["os", "json"]
        })
    ...
    def test_generate_project_summary(self, project_manager, test_project_dir, permission_manager):
...
class TestProjectAnalysis:
    ...
    def test_initialization(self, project_manager, project_analyzer, permission_manager):
        """Test initializing ProjectAnalysis."""
        analysis = ProjectAnalysis(project_manager, project_analyzer, permission_manager)
    ...
    async def test_register_tools(self, project_analysis):
        ...
        with patch.object(project_analysis.project_manager, "set_project_root", return_value=True), \
             patch.object(project_analysis.project_manager, "analyze_project", return_value={}), \
             patch.object(project_analysis.project_manager, "generate_project_summary", return_value="Project summary"):
            ...
            project_analysis.permission_manager.is_path_allowed = MagicMock(return_value=True)
            
            with patch("mcp_claude_code.tools.project.analysis.create_tool_context", return_value=mock_tool_ctx):
                # Call the project_analyze_tool
                result = await tools["project_analyze_tool"]("/test/project/dir", mock_context)
